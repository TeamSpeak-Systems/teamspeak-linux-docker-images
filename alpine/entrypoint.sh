...
if [ "$1" = 'ts3server' ]; then
    ...
    cat << EOF | sed 's/^[ \t]*//;s/[ \t]*$//;/^$/d' > /var/run/ts3server/ts3server.ini
        ...
        query_ip_allowlist=${TS3SERVER_IP_ALLOWLIST:-${TS3SERVER_IP_WHITELIST:-query_ip_allowlist.txt}}
        query_ip_denylist=${TS3SERVER_IP_DENYLIST:-${TS3SERVER_IP_BLACKLIST:-query_ip_denylist.txt}}
